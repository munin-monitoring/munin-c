...
int smart(int argc, char **argv) 
{
    ...
    snprintf(command, sizeof(command), "smartctl -A -d ata /dev/%s -n standby", &basename(argv[0])[6]);
    ...
    while(fgets(output, sizeof(output), f) != NULL) {
        ...
        if(                                         (output[0] >= '0' && output[0] <= '9') ||
                               (output[0] == ' ' && (output[1] >= '0' && output[1] <= '9')) ||
           (output[0] == ' ' && output[1] == ' ' && (output[2] >= '0' && output[2] <= '9'))) {
    ...
    /* Setup for caching */
    snprintf(command, sizeof(command), "/mnt/ram/smart_%s", &basename(argv[0])[6]);
    ...
    if(attribute == 0) {
        ...
        if((f=fopen(command, "r")) == 0) {
            return fail("command did not return data, no cached attribute-list");
            return 0;
        }
    }
    ...
    if(argc > 1) {
        if(strcmp(argv[1], "config") == 0) {
            printf("graph_title S.M.A.R.T values for drive %s\n"
